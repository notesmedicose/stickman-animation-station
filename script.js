...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize multiple projects and load active project
  initProjectsManager();
  ...
  // Initialize responsive layout engine
...
function triggerDownload(url, filename) {
  ...
  if (window.Capacitor && window.Capacitor.isNativePlatform()) {
    console.log("Capacitor platform detected. Initiating native file share/export.");
    ...
    fetch(url)
      ...
      .then(blob => {
        ...
        reader.onloadend = async () => {
          try {
            ...
            if (!Filesystem || !Share) {
              throw new Error("Capacitor Plugins (Filesystem or Share) are not available.");
            }

            // Write the file to a cache directory
            ...
            await Share.share({
              ...
              text: `Check out my animation: ${filename}`,
              url: writeResult.uri,
              dialogTitle: 'Save / Share Animation'
            });
          } catch (err) {
      ...
      .catch(err => {
        ...
        alert("Failed to prepare file: " + err.message);
      });
    return;
  }
...
function saveCurrentProjectData(silent = true) {
  ...
  try {
    ...
    localStorage.setItem(`antigravity_project_${projectState.activeIdx}_data`, JSON.stringify(dataToSave));
    localStorage.setItem('antigravity_projects_meta', JSON.stringify(projectState));

    if (!silent) {
      alert(`"${projectState.projects[projectState.activeIdx].name}" saved successfully inside browser local storage!`);
    }
  } catch (err) {
    console.error("Local Storage Save failed:", err);
    if (!silent) {
      alert("Save failed! Project is too large for browser Local Storage. Try removing some frames or clear drawings.");
    }
  }
}

async function loadProjectData(index) {
  if (state.isPlaying) pauseAnimation();

  // Save current project first (silent)
  saveCurrentProjectData(true);

  projectState.activeIdx = index;
  localStorage.setItem('antigravity_projects_meta', JSON.stringify(projectState));
  ...
  const rawData = localStorage.getItem(`antigravity_project_${index}_data`);
  if (!rawData) {
    // Initialize a blank project for this slot
    state.fps = 8;
    state.canvasBgColor = '#ffffff';
    state.projectRatio = '16:9';
    ...
    state.frames = [new Frame(800, 450)];
    ...
    // Sync UI elements
    const ratioSelect = document.getElementById('project-aspect-ratio');
  ...
  try {
    ...
    if (!project.drawings || project.drawings.length === 0) {
      throw new Error("Project contains no frames");
    }

    state.projectRatio = project.projectRatio || '16:9';
    document.body.classList.remove('ratio-16-9', 'ratio-9-16');
    document.body.classList.add(`ratio-${state.projectRatio.replace(':', '-')}`);
    ...
    for (let i = 0; i < project.drawings.length; i++) {
      ...
      await new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          newFrame.undoStack = [newFrame.ctx.getImageData(0, 0, newWidth, newHeight)];
          resolve();
        };
    ...
    document.querySelectorAll('.bg-option').forEach(opt => {
      if (opt.getAttribute('data-bg') === project.bgColor) {
...
async function loadProjectFromLocalStorage() {
  await loadProjectData(projectState.activeIdx);
  alert("Active project reloaded successfully!");
}

function initProjectsManager() {
  const rawMeta = localStorage.getItem('antigravity_projects_meta');
  if (rawMeta) {
    try {
      const meta = JSON.parse(rawMeta);
      projectState.activeIdx = meta.activeIdx || 0;
      if (meta.projects && meta.projects.length === 3) {
        projectState.projects = meta.projects;
      }
    } catch (e) {
      console.error("Error reading projects metadata:", e);
    }
  }

  // Populate header dropdown selector
  const headerSelect = document.getElementById('header-project-select');
  if (headerSelect) {
    headerSelect.innerHTML = '';
    projectState.projects.forEach((proj, idx) => {
      const opt = document.createElement('option');
      opt.value = idx;
      opt.textContent = proj.name;
      headerSelect.appendChild(opt);
    });
    headerSelect.value = projectState.activeIdx;

    headerSelect.addEventListener('change', (e) => {
      loadProjectData(parseInt(e.target.value));
    });
  }

  // Bind rename project button in header
  const renameHeaderBtn = document.getElementById('btn-rename-current-project');
  if (renameHeaderBtn) {
    renameHeaderBtn.addEventListener('click', () => {
      renameProject(projectState.activeIdx);
    });
  }

  updateProjectsUI();
  loadProjectData(projectState.activeIdx);
}

function renameProject(index) {
  const oldName = projectState.projects[index].name;
  const newName = prompt("Enter a new name for this project:", oldName);
  if (newName && newName.trim() !== "") {
    projectState.projects[index].name = newName.trim();
    localStorage.setItem('antigravity_projects_meta', JSON.stringify(projectState));

    // Update header select options
    const headerSelect = document.getElementById('header-project-select');
    if (headerSelect) {
      headerSelect.options[index].textContent = newName.trim();
    }
...
function updateProjectsUI() {
  ...
  projectState.projects.forEach((proj, idx) => {
    ...
    item.className = `project-sidebar-item ${idx === projectState.activeIdx ? 'active' : ''}`;
    ...
    renameBtn.className = 'btn-icon-only-small';
    renameBtn.title = "Rename Project";
    renameBtn.innerHTML = `<svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"></path></svg>`;
...
function setupUIEvents() {
  ...
  if (btnToggleToolbox && sidebarLeft) {
    btnToggleToolbox.addEventListener('click', (e) => {
      e.stopPropagation();
  ...
  if (btnToggleSettings && sidebarRight) {
  ...
  if (btnCloseLeft && sidebarLeft) {
    btnCloseLeft.addEventListener('click', () => {
      sidebarLeft.classList.remove('open');
    });
  }

  if (btnCloseRight && sidebarRight) {
  ...
  document.addEventListener('click', (e) => {
    if (sidebarLeft && sidebarLeft.classList.contains('open') &&
      ...
      (!btnToggleToolbox || !btnToggleToolbox.contains(e.target))) {
      sidebarLeft.classList.remove('open');
    }
    if (sidebarRight && sidebarRight.classList.contains('open') &&
      !sidebarRight.contains(e.target) &&
      (!btnToggleSettings || !btnToggleSettings.contains(e.target))) {
      sidebarRight.classList.remove('open');
    }
  });
...
function initResponsiveLayout() {
  ...
  window.addEventListener('orientationchange', () => {
    setTimeout(recalculateLayout, 350);
  });
}
...
function recalculateLayout() {
  ...
  const timelineHeight = timelineEl ? timelineEl.offsetHeight : 80;

  // 2. Calculate available space for canvas
  const statusBarHeight = 48; // Height for status bar (badges + undo/redo) + quick controls
  const workspacePadding = 24; // Top+bottom padding in workspace-container
  const availableHeight = vh - headerHeight - timelineHeight - statusBarHeight - workspacePadding;
  const availableWidth = vw - 40; // 20px padding on each side

  // 3. Calculate optimal canvas dimensions maintaining aspect ratio
  const aspectRatio = state.projectRatio === '9:16' ? 9 / 16 : 16 / 9;
  let canvasWidth, canvasHeight;

  if (availableWidth / availableHeight > aspectRatio) {
    // Height constrained
    canvasHeight = Math.max(100, availableHeight);
    canvasWidth = canvasHeight * aspectRatio;
  } else {
    // Width constrained
    canvasWidth = Math.max(100, availableWidth);
    canvasHeight = canvasWidth / aspectRatio;
  }

  // For 9:16 portrait — cap width so it doesn't fill entire screen width
  if (state.projectRatio === '9:16') {
    const maxPortraitWidth = Math.min(vw * 0.45, 420); // max 45% of screen or 420px
    if (canvasWidth > maxPortraitWidth) {
      canvasWidth = maxPortraitWidth;
      canvasHeight = canvasWidth / aspectRatio;
    }
  }
  ...
  if (wrapper) {
    wrapper.style.width = `${Math.round(canvasWidth)}px`;
    wrapper.style.height = `${Math.round(canvasHeight)}px`;
  }

  // 5. Update portrait lock - only for narrow portrait screens
  const lockOverlay = document.getElementById('portrait-lock-overlay');
  if (lockOverlay) {
    if (vw < 500 && vh > vw) {
      lockOverlay.style.display = 'flex';
    } else {
      // Only hide if not forced by CSS media query
      if (vw >= 500 || vh <= vw) {
